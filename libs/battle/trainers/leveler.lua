...
function Leveler:_onPathAction()
    ...
    then
        ...
        local new_starter_id = TeamManager.getLowestAlivePkmToLvl(self.lvl_cap)
    ...
    log("DEBUG | leftovers target: "..tostring(leftovers_target))
...
function Leveler:_onBattleAction()
    ...
    -- switch in available
    if lvl_dif < self:getMinLevelAdv(index) or getPokemonHealth(id_active) == 0 then
...
function Leveler:getSwitchIn()
    for _, index in pairs(TeamManager.getUsablePkmToLvl(self.lvl_cap)) do
        ...
        if lvl_dif >= self:getMinLevelAdv(index) then return index end
    end
end
...
function Leveler:getMinLevelAdv(index) 
    --TODO: check for typ advantage(-5) or disadvantage(+5)
    --TODO: trace last hit hp lost, swapout if not surviving
    return 8
end


--TODO: check movepool of
