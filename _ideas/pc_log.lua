...
natures =
{
    ...
    Naughty = "+Attack -SpDefence",
    Brave = "+Attack -Speed",

    Bold = "+Defence -Attack",
    Impish = "+Defence -SpAttack",
    Lax = "+Defence -SpDefence",
    Relaxed = "+Defence -Speed",

    Modest = "+SpAttack -Attack",
    Mild = "+SpAttack -Defence",
    Rash = "+SpAttack -SpDefence",
    Quiet = "+SpAttack -Speed",

    Calm = "+SpDefence -Attack",
    Gentle = "+SpDefence -Defence",
    Careful = "+SpDefence -SpAttack",
    Sassy = "+SpDefence -Speed",
    ...
    if getCurrentPCBoxId() == currentBoxId then
        ...
        for i = 1, currentBoxSize do
            if getTextOption(2) == "" or getPokemonNameFromPC(currentBoxId, i) == getTextOption(2) then
                ...
                append("		Nature: " .. getPokemonNatureFromPC(currentBoxId, i) .. " (" .. (natures[getPokemonNatureFromPC(currentBoxId, i)] or "Unknown") .. ")")
...
function getMoves(boxIndex, pokeIndex)
    ...
    for i = 1, 4 do
        if getPokemonMoveNameFromPC(boxIndex, pokeIndex, i) then
            ...
            append("			Type: " .. getPokemonMoveTypeFromPC(boxIndex, pokeIndex, i):title())
            append("			Damage Type: " .. getPokemonMoveDamageTypeFromPC(boxIndex, pokeIndex, i))
