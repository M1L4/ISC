...
--Need Leftover,so work. it will automatically give it to your first usable pokemon.--
setOption(2, true)
setOptionName(2, "Snack")
setOptionDescription(2, "leftovers Needed or Off = No to leftovers, On = Use Leftovers")

setOption(3, false)
setOptionName(3, "PM's")
setOptionDescription(3, "Private Message on=on off=off")
...
stopAtLevel = 50
powerLevelingPokemon = "Bagon"

--Case Sensitive--
--Put the name of the map you want to train at between "". {Example: location = "Route 121"}
local location = "Dragons Den"		

-- Put "Exit" to move around exit, Grass" for grass, "Water" for water, fish{x, y} for fishing cell, {x1, y1, x2, y2} for rectangle
...
local catchThesePokemon = {"Riolu", "Scyther", "Dragonair", "Dratini", "Bagon", "Nosepass"}
...
--Leave an empty "" here if you aren't using it--
local movesNotToForget = {"Dig", "Cut", "Surf", "Flash", "Rock Smash", "Dive", "Headbutt"}
...
local pf = require "externas.proshinepathfinder.PathFinder.Lib.lib"
local Lib = require "externas.proshinepathfinder.PathFinder.Lib.lib"
local map = nil
...
function onBattleMessage(wild)
	if getOption(3) then enablePrivateMessage() else disablePrivateMessage() end
		...
		if getTotalUsablePokemonCount() >= 1 and (getTotalPokemonToLevelCount() > 1 or (getTotalPokemonToLevelCount() == 1 and getPokemonHealthPercent(getFirstPokemonToLevel()) >= healthToRunAt)) then
			if getMapName() == location then
				...
				elseif location == "WATER" then
					return moveToWater()
				elseif location == "Exit" then
					return moveNearExit("Granite Cave B1F")
				end
...
function onBattleAction()
		...
		if isWildBattle() and getOption(1) and not isOnList(evadeThesePokemon) then
